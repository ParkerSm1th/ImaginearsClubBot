...
module.exports = {
  ...
  logCommand: function (message, cmd) {
    console.log(chalk.cyan(`(${getTime()}) [LOG] ` + message.author.username + " (" + message.author.id + ") has successfully run the command " + cmd + " in " + message.channel.name));
  },

  logErrorCommand: function (message, cmd) {
    console.log(chalk.cyan(`(${getTime()}) [LOG] ` + message.author.username + " (" + message.author.id + ") has tried to run the command " + cmd + " in " + message.channel.name + " (Command not found)"));
  }
}
